...
На 13.09.2026 проект содержит **714 проверок на каждую версию PowerShell**: 5.1 и 7. Guard (61) проверяется с реальными файловыми блокировками, отчётами и историей; запускатель (57) — настоящими процессами PowerShell с безопасными сценариями и передачей кодов возврата. Обновление готовой VM проверено на временных файлах с подменой планировщика. Набор обслуживания (55) проверяет WIM-метаданные, подготовку и кэш инструментов, параметры аккаунта и answer-файл. Системные API, сеть и DISM в соответствующих тестах подменены. Полная установка 26H1 в VM остаётся отдельной проверкой.
