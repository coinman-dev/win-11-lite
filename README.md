...
Служебные скрипты установки запускаются через небольшой `Win11Lite.Run.exe`, который не создаёт консоль. Сборщик заранее компилирует этот GUI-помощник из открытого исходника [`data/SetupLauncher.cs`](data/SetupLauncher.cs) штатным `csc.exe` из .NET Framework и сохраняет в кэше. Основная логика сборки, Prepare, Finalize и guard остаётся на PowerShell; EXE запускает эти сценарии через `CreateNoWindow`, поскольку один `-WindowStyle Hidden` может дать короткую вспышку консоли. Дополнительная загрузка или установка SDK для помощника не нужна.
...
Окно журнала guard открывается после завершения OOBE. Вызовы подготовки, финализации и фонового guard выполняются без промежуточного окна CMD/PowerShell. Их запуск и коды возврата записываются в `C:\Windows\Setup\Scripts\Win11Lite\launcher.log`.
...
На 13.09.2026 проект содержит **699 проверок на каждую версию PowerShell**: 5.1 и 7. Набор guard (49) включает реальные открытые файловые дескрипторы для совместного чтения и исключительной блокировки журнала. Набор обслуживания (55) проверяет WIM-метаданные, подготовку и кэш инструментов, параметры аккаунта и answer-файл. Наборы выполняются отдельными процессами; системные API, сеть и DISM в соответствующих тестах подменены. Тест запуска компилирует настоящий GUI EXE и запускает безопасные сценарии с проверкой отсутствия консоли и передачи кодов возврата. Полная установка 26H1 в VM остаётся отдельной проверкой.
...
Реальные русские WinPE CAB проверены, DryRun исходного ISO прошёл. Журналы VM сборок `20260912-203504` и `20260912-225013` подтверждают ожидание OOBEComplete и последующий возврат сети. В `225013` также подтверждены успешные запуски Prepare/Finalize через GUI-помощника. Два запуска guard завершились без ошибок; при более позднем входе выявлен конфликт записи `Add-Content` с чтением журнала, исправленный 13.09.2026. Исправление проверено на PS 5.1/7, повторный запуск в VM после замены guard ещё предстоит. Изменения: [`CHANGELOG.md`](CHANGELOG.md).
