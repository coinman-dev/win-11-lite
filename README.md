...
Обновление guard в уже установленной Windows выполняется пересборкой ISO: отдельного средства обновления больше нет. Режим окна выбирается при сборке через `-GuardMode Standard|Debug|Silent` и хранится в `guard.json`.
