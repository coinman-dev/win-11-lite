...
Ярлык `Install-Firefox` запускает установку браузера уже в установленной Windows; для этого ему потребуется интернет. Команда winget использует источник `winget` явно, чтобы поиск Firefox не зависел от доступности `msstore`. При отказе сохраняется прямая загрузка с Mozilla. После успешной установки одноразовый ярлык удаляется. При ошибке скачивания или отмене установки он остаётся для повторного запуска; обычный ярлык Firefox сохраняется.
