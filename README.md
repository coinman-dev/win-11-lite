...
На 14.09.2026 проект содержит **1020 проверок на каждую версию PowerShell**: 5.1 и 7. Все 12 наборов прошли. Затронутые режимы guard, запускатель и обслуживание проверяются с подменой системных API и безопасными временными файлами/процессами. Реальный DryRun ISO 28000.2704 Pro прошёл с `-Guard None`, `Standard`, `Debug` и `Silent`; полная сборка после объединения параметров отдельно не выполнялась.
