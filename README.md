...
Служебные скрипты установки запускаются через небольшой `Win11Lite.Run.exe`, который не создаёт консоль. Сборщик заранее компилирует его из `data/SetupLauncher.cs` штатным компилятором .NET Framework и сохраняет в кэше. Дополнительная загрузка или установка SDK не нужна.
...
Реальные русские WinPE CAB проверены, DryRun исходного ISO прошёл. В полученных журналах VMware сборки `20260912-203504` подтверждены блокировка сети во время OOBE, ожидание после первого входа и возврат сети после OOBEComplete. Guard завершился без ошибок. Новый способ запуска без мигания консоли проверен отдельно; его поведение в полной установке ещё требует проверки после пересборки. Изменения: [`CHANGELOG.md`](CHANGELOG.md).
