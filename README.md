...
Без `-WithUpdates` и `-WithWinget` последние обновления и winget не встраиваются. Ярлык `Install-Firefox` запускает установку браузера уже в установленной Windows; для этого ему потребуется интернет.
...
На 12.09.2026 прошли **368 проверок на каждой версии PowerShell**: 5.1 и 7. Наборы выполняются отдельными процессами; системные API, сеть и DISM в соответствующих тестах подменены.

```powershell
$suites = 'Test-Win11Lite', 'Test-WindowsBatch', 'Test-Guard', 'Test-Downloads', 'Test-SetupLanguage'
...
Реальные русские WinPE CAB проверены, DryRun исходного ISO прошёл. Полная сборка и установка последней редакции с переводом установщика в VM пока не подтверждены. Сводка: [`.ai/audit-validation.json`](.ai/audit-validation.json); изменения: [`CHANGELOG.md`](CHANGELOG.md).
