...
На 13.09.2026 проект содержит **768 проверок на каждую версию PowerShell**: 5.1 и 7. Guard (77) проверяется с реальными файловыми блокировками, отчётами, историей и подменёнными системными операциями. GuardModes (25) проверяет краткие счётчики, выбор пользовательской сессии, ID отчёта, Silent и ограниченную повторную попытку удаления. Запускатель (66) проверяется настоящими процессами с безопасными сценариями; обновление готовой VM — на временных файлах с подменой планировщика. Набор обслуживания (59) проверяет WIM-метаданные, подготовку инструментов, параметры аккаунта и answer-файл. Новые режимы требуют проверки отображения в VM.

```powershell
$suites = 'Test-Win11Lite', 'Test-WindowsBatch', 'Test-Guard', 'Test-GuardModes', 'Test-Downloads', 'Test-SetupLanguage', 'Test-OobeNetwork', 'Test-SetupLauncher', 'Test-Compatibility', 'Test-Servicing'
